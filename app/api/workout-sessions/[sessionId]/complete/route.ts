...
export async function POST(request: Request, context: { params: Promise<{ sessionId: string }> }) {
  ...
  if (!current || (current.status !== "ACTIVE" && current.status !== "PAUSED")) {
    return Response.json({ error: "In-progress Session not found" }, { status: 404 });
  }
  const completedAt = new Date();
  const finalPauseMs = current.status === "PAUSED" && current.pausedAt
    ? Math.max(0, completedAt.getTime() - current.pausedAt.getTime())
    : 0;
